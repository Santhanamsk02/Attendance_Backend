...
def migrate():
    ...
    try:
        ...
        pg_db.query(TeacherAssignment).filter(TeacherAssignment.department_id == it_pg_id).delete()
        pg_db.query(TimetableStructure).filter(TimetableStructure.department_id == it_pg_id).delete()
        pg_db.commit()
        
        target_sections = ['A', 'B', 'C', 'D', 'E']

        # 5. Migrate Teacher Assignments
        print("Migrating TeacherAssignments...")
        sq_assignments = sq_db.query(TeacherAssignment).filter(
            TeacherAssignment.department_id == it_sq_id,
            TeacherAssignment.section_id.in_(target_sections)
        ).all()
        ...
        for sa in sq_assignments:
            ...
            new_year_id = year_map.get(sa.academic_year_id)
            
            if not new_teacher_id or not new_subject_id or not new_year_id:
            ...
            pg_assignment = TeacherAssignment(
        ...
        # 6. Migrate Timetable Structures
        print("Migrating TimetableStructures...")
        sq_structures = sq_db.query(TimetableStructure).filter(
            TimetableStructure.department_id == it_sq_id,
            TimetableStructure.section_name.in_(target_sections)
        ).all()
        
        migrated_structures = 0
        
        for st in sq_structures:
            new_dept_id = dept_map.get(st.department_id)
            new_batch_id = batch_map.get(st.batch_id)
            
            if not new_dept_id or not new_batch_id:
                continue
                
            # Deep copy and update grid_data to map teacher_ids
            import copy
            new_grid = copy.deepcopy(st.grid_data)
            if new_grid and isinstance(new_grid, dict):
                for d in new_grid.get("days", []):
                    for entry in d.get("entries", []):
                        old_tid = entry.get("teacher_id")
                        if old_tid and old_tid in teacher_map:
                            entry["teacher_id"] = teacher_map[old_tid]
                            
            pg_structure = TimetableStructure(
                department_id=new_dept_id,
                batch_id=new_batch_id,
            ...
            pg_db.add(pg_structure)
            migrated_structures += 1
            
        print("Committing changes to Postgres DB...")
        pg_db.commit()
        
        print(f"Successfully migrated {migrated_assignments} TeacherAssignments (Skipped: {skipped_assignments}).")
        print(f"Successfully migrated {migrated_structures} TimetableStructures.")
